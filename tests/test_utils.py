...
class DummyTrainer:
    def __init__(
        self, limit_train_batches, num_training_batches, max_epochs, max_steps=None
    ):
...
class UtilsTestCase(unittest.TestCase):
    def test_get_num_training_batches_int_limit(self):
        trainer = DummyTrainer(
            limit_train_batches=5, num_training_batches=10, max_epochs=1
        )
        self.assertEqual(utils.get_num_training_batches(trainer), 5)

    def test_get_num_training_batches_float_limit(self):
        trainer = DummyTrainer(
            limit_train_batches=0.5, num_training_batches=20, max_epochs=1
        )
    ...
    def test_get_max_epochs_no_steps(self):
        trainer = DummyTrainer(
            limit_train_batches=1.0, num_training_batches=4, max_epochs=3
        )
